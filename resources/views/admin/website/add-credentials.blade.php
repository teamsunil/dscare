                    ...
                    <div class="panel-body">
                            ...
                            <div class="alert alert-danger">
                                <ul class="list-unstyled">
                                    @foreach ($errors->all() as $error)
                                        <li><span class="glyphicon glyphicon-exclamation-sign"></span> {{ $error }}
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                        ...
                        <form id="add-credentials-form" action="{{ route('website.submit.credentials') }}" method="POST"
                            ...
                            <div class="form-group">
                                <label for="username" class="col-xs-12 col-sm-3 col-md-2 control-label">Email or
                                    Username</label>
                            ...
                            <input type="hidden" name="site_url" id="site_url" value="{{ $websiteUrl }}" required>
                            <input type="hidden" name="shared_secret" id="shared_secret" value="{{ $sharedSecret ?? '' }}"
                                required>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                form.addEventListener('submit', function(e) {
                    ...
                    submitBtn.innerHTML =
                        '<span class="glyphicon glyphicon-refresh glyphicon-refresh-animate"></span> Verifying...';
                    ...
                    fetch(siteUrl + '/wp-json/laravel-sso/v1/check-login', {
                        ...
                        .then(async response => {
                            ...
                            if (response.ok && data.success) {
                                ...
                                const addTokenUrl = siteUrl +
                                    '/wp-json/laravel-sso/v1/add-secret-token?token=' +
                                    encodeURIComponent(sharedSecret) + '&url=' + encodeURIComponent(
                                ...
                                fetch(addTokenUrl, {
                                    ...
                                    .then(async addResp => {
                                        ...
                                        if (!addResp.ok) {
                                            ...
                                            throw new Error(
                                                'Failed to register token on remote site. ' +
                                                (txt || addResp.statusText));
                                        }
                                        // Token registered successfully — submit the form to save credentials on our side
                                        setTimeout(() => form.submit(), 500);
                                    })
                                    .catch(err => {
                                        showError('Could not register token on the remote site: ' +
                                            (err.message || err));
                        ...
                        .catch(() => {
                            showError(
                                'Could not connect to the site or API. Please check the URL and try again.'
                                );
                            submitBtn.innerHTML = originalBtnText;
                            submitBtn.disabled = false;
                        });
